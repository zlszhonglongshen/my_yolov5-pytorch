...
class My_YOLO_backbone_head(nn.Module):
    def __init__(self, num_classes, num_anchors):
        ...
        self.seq2_Bottleneck = Bottleneck(128, 128)
        self.seq3_Conv = Conv(128, 256, 3, 2)
        self.seq4_BottleneckCSP = BottleneckCSP(256, 256, 9)
        self.seq5_Conv = Conv(256, 512, 3, 2)
        self.seq6_BottleneckCSP = BottleneckCSP(512, 512, 9)
        self.seq7_Conv = Conv(512, 1024, 3, 2)
        self.seq8_SPP = SPP(1024, 1024, [5, 9, 13])
        self.seq9_BottleneckCSP = BottleneckCSP(1024, 1024, 6)
        self.seq10_BottleneckCSP = BottleneckCSP(1024, 1024, 3, False)
        self.seq11_Conv2d = nn.Conv2d(1024, self.no, 1, 1, 0)
        self.seq14_Conv = Conv(1536, 512, 1, 1)
        self.seq15_BottleneckCSP = BottleneckCSP(512, 512, 3, False)
        self.seq16_Conv2d = nn.Conv2d(512, self.no, 1, 1, 0)
        self.seq19_Conv = Conv(768, 256, 1, 1)
        self.seq20_BottleneckCSP = BottleneckCSP(256, 256, 3, False)
        self.seq21_Conv2d = nn.Conv2d(256, self.no, 1, 1, 0)
    def forward(self, x):
        x = self.seq0_Focus(x)
        x = self.seq1_Conv(x)
